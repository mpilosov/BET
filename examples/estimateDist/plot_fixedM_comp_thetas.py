...
# theta_range = [0,6,12]
line_colors = np.linspace(0.8, 0, len(theta_range))
...
for M_idx in range(len(M_vec)):
    ...
    for theta_idx in range( len(theta_range)) :
        ...
        # H = np.load('rot/dict_orth_results_theta_%d.npy'%theta) # orthogonal map rotated through 0 .. 90 (resolution = 15)
        # H = np.load('rot/(%d,%d)_dict_rot_results_theta_%d.npy'%(alpha, beta, theta) ) # 45 deg skewed map rotated 0 .. 90 (resolution = 5)
        H = H.item()
    ...
    if var_or_mean == 'mean':
        plt.axis([20, 7500, 1E-3, 1])
        plt.savefig('(%d,%d)_Mean_M%d.png'%(alpha, beta, M**2) )
    else:
        plt.axis([20, 7500, 1E-6, 1E-2])
        plt.savefig('(%d,%d)_Var_M%d.png'%(alpha, beta, M**2) )
