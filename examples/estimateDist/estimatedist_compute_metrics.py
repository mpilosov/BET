...
# ALL this does is compute distances.
...
for BigN in BigN_values:  # reference solution resolution
    ...
    for M in M_values:  # data space discretization mesh
        ...
        for I in I_values:  # integration mesh
            ...
            data_dict = {
                (N**(1 + (dim_input-1)*(estimate_mesh_type == 'reg'))): {} for N in N_values}
            ...
            for N in pyprind.prog_percent(N_values):
                ...
                for trial in range(num_trials):
                    ...
                    # number of possible QoIs
                    for sol_num in range(len(QoI_choice_list)):
                        est_sol_dir_2 = est_sol_dir + \
                            'QoI_choice_%d' % (sol_num+1) + '/'
                        ...
                        mc_val, C = mc_TV(Integration_Set,
                                            Ref_Disc, ref_ptr,
                                            Est_Disc, est_ptr)
                        temp_array[trial, sol_num] = mc_val
...
#                                 C, 'results_heatrod_3/diff_t%d_N%d_sol%d' % (trial, N, sol_num))
                ...
                data_dict[Nval]['stats'] = [
                    np.mean(temp_array, axis=0), np.var(temp_array, axis=0)]
