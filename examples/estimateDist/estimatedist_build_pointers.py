    ...
    for BigN in BigN_values:
        ...
        for int_mesh_num in range(len(I_values)):
            ...
            integration_sets_filename =  cwd +  '/' + results_dir + '/' + \
                    sub_dirs[1] + '/' + '%s_IntSet_%d'%(integration_mesh_type, Ival)
...
if create_est_discs or create_int_sets:
    ...
    for N in N_values:
        ...
        est_disc_dir = cwd + '/' + results_dir + '/' + sub_dirs[2] + '/' + '%s_N_%d'%(estimate_mesh_type, Nval) + '/' # all trial discretizations inside this folder    
        print('\t Working on N = %d ... '%(Nval))
        for trial in range(num_trials):
            est_disc_filename = est_disc_dir + '%s_N_%d'%(estimate_mesh_type, Nval) + '_trial_%d'%(trial)
